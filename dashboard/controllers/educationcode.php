...
if (isset($_POST['del_education_btn'])) {
    $edu_id = mysqli_real_escape_string($conn, $_POST['del_education_btn']);

    $edu_delete_query = mysqli_query($conn, "DELETE FROM qualifications WHERE id='$edu_id'");
...
if (isset($_POST['update_education_btn'])) {
    ...
    $query = mysqli_query($conn, "UPDATE qualifications SET college_name='$college_name',level='$degree',description='$desc',completion_year='$end_year',board_or_university='$brd_uni',percentage_or_cgpa='$percent_or_gpa',status='$status' WHERE id='$edu_id'");

    if ($query) {
        $_SESSION['success'] = 'Updated Successfully';
        header('Location: ../edit-education?id=' . $edu_id);
        exit(0);
    } else {
        $_SESSION['failure'] = 'Something went wrong';
        header('Location: edit-education?id=' . $edu_id);
...
if (isset($_POST['education_visibility_btn'])) {
    ...
    $skill_id = $_GET['id'];
    if ($status == '1') {
        $query = mysqli_query($conn, "UPDATE qualifications SET status='0' WHERE id='$skill_id' LIMIT 1");
        $_SESSION['success'] = 'Education is hidden';
        header('Location: ../view-educations');
        exit(0);
    } else {
        $query = mysqli_query($conn, "UPDATE qualifications SET status='1' WHERE id='$skill_id' LIMIT 1");
